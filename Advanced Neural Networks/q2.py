...
class NeuralNetwork(object):
		...
		for x in range(100):
			...
			for p in patterns:
				...
				if count < len(patterns)*(90/100): #Only train the network or the first 90% of the data
					...
					pass
				pass
			print('error %-.4f' % (error/len(patterns)))
			total_error += error
	...
	def backPropagation(self, targets):
		...
		for i in range(self.num_outputs):
			...
			layer2_partial_derivatives[i] = ReLu_derivative(self.ao[i]) * error#step2
...
def demo():
	...
	act = int(input("Select Activation\n1. sigmoid\n2. Tanh\n3. ReLu\n"))
