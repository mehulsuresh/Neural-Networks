...
class NeuralNetwork(object):
	...
	def __init__(self, num_inputs, num_hidden, num_outputs):
		...
		for i in range(self.num_inputs):
			for j in range(self.num_hidden):
				self.w1[i][j] = rand(-1/num_inputs, 1/num_inputs)
				pass
			pass
		for j in range(self.num_hidden):
			for k in range(self.num_outputs):
				self.w2[j][k] = rand(-1/num_hidden, 1/num_hidden)
				pass
			pass
	...
	def calculate(self, inputs):
		...
		for k in range(self.num_outputs):
			...
			self.ao[k] = softplus_activation(sum)
			pass
		return self.ao[:]
	def trainNetwork(self, patterns):
		...
		for x in range(100):
			...
			total_error +=error
			pass
		print('Average Error is ', total_error/100)	
		pass
	...
	def backPropagation(self, targets):
		...
		for i in range(self.num_outputs):
			...
			layer2_partial_derivatives[i] = softplus_derivative(self.ao[i]) * error#step2
